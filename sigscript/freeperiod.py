...
#    lgrad = np.gradient(sense,80)
                                      # lgrad is the derivative of the laser: Scan for zero crossings
#    lgfilt = sp.signal.symiirorder1(lgrad,4,.8)
#    Z = []                            # Z is the list of laser values corresponding to where the derivative flips sign,
#    ZZ = []                           # and should represent where the local max/min occurred in the waveform.
#    offset =  2738              
#    polarity = 0
#    zero_crossings = np.where(np.diff(np.sign(lgfilt)))[0]  
#    for i in range(0,len(zero_crossings)):
#        Z.append(sense[zero_crossings[i]]+offset)
#                                     #    print sense[zero_crossings[i]]
#    for i in range(0,len(Z)-1):
#        zz = np.abs(Z[i-1])
#        ZZ.append(zz)
#                                     # Adjust Z for middle tail-end to fix bias problems
#                                     # Z = Z[:]-np.mean(Z[1:len(Z)-5])
#    hh = []
#    for i in range(1,len(Z)):              # calculate a list of damping ratios starting with the second local max
#        hn = np.log(np.abs(float(Z[i-1])/Z[i])) / np.sqrt(np.square(np.pi)+np.log(np.abs(float(Z[i-1])/Z[i])))
#        print i,Z[i-1],Z[i],float(Z[i-1])/Z[i],hn
#        hh.append(hn)
#    # print np.min(hh)
#    #x = range(0,len(ZZ))
#    # graph_curve(x[5:22],np.abs(Z[5:22]))
#    # print Z[5:22]
#    x = range(0,len(hh))                                      
#    graph_curve(x,hh)
#    # print len(Z),np.mean(Z[10:len(Z)-5])
